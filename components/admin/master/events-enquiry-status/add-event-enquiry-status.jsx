...
const AddEventEnquiryStatus = ({ type }) => {
    ...
    const getEnquiryStatus = async (id) => {
        try {
            const response = await instance.get(`/eventsEnquiryStatus?id=${id}`);
            ...
            console.error("Failed to load Enquiry Status:", error);
            showError("Failed to load Enquiry Status data");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/eventsEnquiryStatus?id=${id}`, payload);

                showSuccess(response?.data?.message || "Enquiry Status updated successfully!");
            } else {
                response = await instance.post("/eventsEnquiryStatus", payload);
                showSuccess(response?.data?.message || "Enquiry Status created successfully!");
            }
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View Enquiry Status" : `${type} Enquiry Status`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
                    ...
                    {type !== "View" && (
                        <div className="flex justify-end mt-4">
                            ...
                            >
                                ...
                                {type === "Edit" ? "Update Enquiry Status" : "Add Enquiry Status"}
                            </Button>
                        </div>
                    )}
                </form>
...
export default AddEventEnquiryStatus;
