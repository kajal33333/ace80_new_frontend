...
// Filter-related states
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await instance.get(`/enquiryModels?${queryParams.toString()}`);
  ...
  const deleteReport = async () => {
    try {
      const response = await instance.delete(`/enquiryModels?id=${deleteId}`);
      if (response?.status === 200) {
  ...
  const renderActions = (report) => (
    <div className="flex gap-2">
      ...
      <Link href={`/admin/view-model?id=${report.id}`} className="text-blue-600 hover:text-blue-800">
        <Eye size={16} />
      </Link>

     
      <Link href={`/admin/edit-model?id=${report.id}`} className="text-yellow-600 hover:text-yellow-800">
        <Edit size={16} />
      </Link>
