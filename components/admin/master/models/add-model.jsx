...
const AddModel = ({ type }) => {
    ...
    const getModel = async (id) => {
        try {
            const response = await instance.get(`/enquiryModels?id=${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/enquiryModels?id=${id}`, payload);

                showSuccess(response?.data?.message || "Model updated successfully!");
            } else {
                response = await instance.post("/enquiryModels", payload);
                showSuccess(response?.data?.message || "Model created successfully!");
            }
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View model" : `${type} Model`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
                    ...
                    {type !== "View" && (
                        <div className="flex justify-end mt-4">
                            ...
                            >
                                ...
                                {type === "Edit" ? "Update Model" : "Add Model"}
                            </Button>
                        </div>
                    )}
                </form>
