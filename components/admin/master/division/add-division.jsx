...
const AddDivision = ({ type }) => {
    ...
    const getDivision = async (id) => {
        try {
            const response = await instance.get(`/enquiryDivision?id=${id}`);
            ...
            showError("Failed to load division data");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/enquirydivision?id=${id}`, payload);

                showSuccess(response?.data?.message || "Division updated successfully!");
            } else {
                response = await instance.post("/enquiryDivision", payload);
                showSuccess(response?.data?.message || "Division created successfully!");
            }
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View Division" : `${type} Division`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
                    ...
                    {type !== "View" && (
                        <div className="flex justify-end mt-4">
                            ...
                            >
                                ...
                                {type === "Edit" ? "Update Division" : "Add Division"}
                            </Button>
                        </div>
                    )}
                </form>
