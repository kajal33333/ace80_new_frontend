...
const AddState = ({ type }) => {
    ...
    const getState = async (id) => {
        try {
            const response = await instance.get(`/enquiryState?id=${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/enquiryState?id=${id}`, payload);

                showSuccess(response?.data?.message || "State updated successfully!");
            } else {
                response = await instance.post("/enquiryState", payload);
                showSuccess(response?.data?.message || "State created successfully!");
            }
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View state" : `${type} State`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
