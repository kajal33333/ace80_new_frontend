...
// Filter-related states
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await instance.get(`/enquiryCompetitorName?${queryParams.toString()}`);
  ...
  const deleteReport = async () => {
    try {
      const response = await instance.delete(`/enquiryCompetitorName?id=${deleteId}`);
      if (response?.status === 200) {
  ...
  const renderActions = (report) => (
    <div className="flex gap-2">
      ...
      <Link href={`/admin/view-competitor?id=${report.id}`} className="text-blue-600 hover:text-blue-800">
        <Eye size={16} />
      </Link>

     
      <Link href={`/admin/edit-competitor?id=${report.id}`} className="text-yellow-600 hover:text-yellow-800">
        <Edit size={16} />
      </Link>
  ...
  <Link href="/admin/add-competitor">
    <Button variant="default" size="sm" className="gap-2">
      <IconPlus size={16} /> Add Competitor
    </Button>
  </Link>
