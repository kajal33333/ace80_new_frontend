...
const AddTodoStatus = ({ type }) => {
    ...
    const getLeaveOdType = async (id) => {
        try {
            const response = await instance.get(`/todoStatus?id=${id}`);
            ...
            console.error("Failed to load todoStatus:", error);
            showError("Failed to load todoStatus data");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/todoStatus?id=${id}`, payload);

                showSuccess(response?.data?.message || " updated successfully!");
            } else {
                response = await instance.post("/todoStatus", payload);
                showSuccess(response?.data?.message || " created successfully!");
            }

            router.push("/admin/todo-status");
        } catch (error) {
            console.error(error);
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View todoStatus" : `${type} todoStatus`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
                        onClick={() => router.push("/admin/todo-status")}
                        className="gap-2"
                    >
                        <ArrowLeft size={16} />
                    ...
                    {type !== "View" && (
                        <div className="flex justify-end mt-4">
                            ...
                            >
                                ...
                                {type === "Edit" ? "Update TodoStatus" : "Add TodoStatus"}
                            </Button>
                        </div>
                    )}
                </form>
