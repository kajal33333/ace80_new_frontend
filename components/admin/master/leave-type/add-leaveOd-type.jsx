...
const AddLeaveOdType = ({ type }) => {
    ...
    const getLeaveOdType = async (id) => {
        try {
            const response = await instance.get(`/leaveOdType?id=${id}`);
            ...
            console.error("Failed to load leave/OdType:", error);
            showError("Failed to load leave/OdType data");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/leaveOdType?id=${id}`, payload);

                showSuccess(response?.data?.message || " updated successfully!");
            } else {
                response = await instance.post("/leaveOdType", payload);
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View leaveOd/Type" : `${type} leaveOd/Type`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
                    ...
                    {type !== "View" && (
                        <div className="flex justify-end mt-4">
                            ...
                            >
                                ...
                                {type === "Edit" ? "Update Leave/OdType" : "Add Leave/OdType"}
                            </Button>
                        </div>
                    )}
                </form>
