...
const AddRegion = ({ type }) => {
    ...
    const getRegion = async (id) => {
        try {
            const response = await instance.get(`/enquiryRegion?id=${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/enquiryRegion?id=${id}`, payload);

                showSuccess(response?.data?.message || "Region updated successfully!");
            } else {
                response = await instance.post("/enquiryRegion", payload);
                showSuccess(response?.data?.message || "Region created successfully!");
            }
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View region" : `${type} Region`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
                        onClick={() => router.push("/admin/region")}
                        className="gap-2"
                    >
                        <ArrowLeft size={16} />
                    ...
                    {type !== "View" && (
                        <div className="flex justify-end mt-4">
                            ...
                            >
                                ...
                                {type === "Edit" ? "Update region" : "Add Region"}
                            </Button>
                        </div>
                    )}
                </form>
