...
// Filter-related states
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await instance.get(`/todoPriority?${queryParams.toString()}`);
  ...
  const deleteReport = async () => {
    try {
      const response = await instance.delete(`/todoPriority?id=${deleteId}`);
      if (response?.status === 200) {
  ...
  const renderActions = (report) => (
    <div className="flex gap-2">
      ...
      <Link href={`/admin/view-todopriority?id=${report.id}`} className="text-blue-600 hover:text-blue-800">
        <Eye size={16} />
      </Link>

     
      <Link href={`/admin/edit-todopriority?id=${report.id}`} className="text-yellow-600 hover:text-yellow-800">
        <Edit size={16} />
      </Link>
  ...
  <Link href="/admin/add-todopriority">
    <Button variant="default" size="sm" className="gap-2">
      <IconPlus size={16} /> Add Todo Priority
    </Button>
  </Link>
