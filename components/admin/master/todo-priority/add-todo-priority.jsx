...
const AddTodoPriority = ({ type }) => {
    ...
    // Fetch division data
    const getLeaveOdType = async (id) => {
        try {
            const response = await instance.get(`/todoPriority?id=${id}`);
            ...
            console.error("Failed to load todopriority:", error);
            showError("Failed to load todopriority data");
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (type === "Edit") {
                await instance.put(`/todoPriority?id=${id}`, payload);

                showSuccess(response?.data?.message || " updated successfully!");
            } else {
                response = await instance.post("/todoPriority", payload);
                showSuccess(response?.data?.message || " created successfully!");
            }

            router.push("/admin/todo-priority");
        } catch (error) {
            console.error(error);
            ...
            <div className="bg-white dark:bg-background border shadow rounded-lg p-6">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-gray-200">
                        {type === "View" ? "View todoPriority" : `${type} todoPriority`}
                    </h2>
                    <Button
                        variant="default"
                        size="sm"
                        onClick={() => router.push("/admin/todo-priority")}
                        className="gap-2"
                    >
                        <ArrowLeft size={16} />
                    ...
                    {type !== "View" && (
                        <div className="flex justify-end mt-4">
                            ...
                            >
                                ...
                                {type === "Edit" ? "Update TodoPriority" : "Add TodoPriority"}
                            </Button>
                        </div>
                    )}
                </form>
...
export default AddTodoPriority;
